...
class HyperparamOpt():
    ...
    def objective(self,trial):
        ...
        if classifier_name == "SVC":
            ...
            classifier_obj = sklearn.linear_model.LinearRegression(normalize=normalize)
            ...
            classifier_obj = sklearn.linear_model.LogisticRegression(penalty=penalty,max_iter=max_iter)
        elif classifier_name == "Lasso":
            alpha = trial.suggest_uniform("alpha",0,1)
            classifier_obj = sklearn.linear_model.Lasso(alpha=alpha)
        elif classifier_name == "DecisionTree":
            min_samples_leaf = trial.suggest_int("min_samples_leaf",1,3)
            classifier_obj = sklearn.tree.DecisionTreeClassifier(min_samples_leaf=min_samples_leaf)
        else:
        ...
        score = sklearn.model_selection.cross_val_score(classifier_obj,
                                                        self.x, self.y, n_jobs=-1,
                                                        cv=cross_validation_k)
        accuracy = score.mean()
        return accuracy

    def get_best_params(self,n_trials):
        study = optuna.create_study(pruner="PercentilePruner", direction="maximize",study_name="Ankan Study")
